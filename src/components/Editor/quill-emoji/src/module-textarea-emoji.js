...
/* Gère l'ajout d'emoji depuis le module dans la textarea */
...
class TextAreaEmoji extends Module {
  ...
  checkEmojiBoxExist() {
    ...
    if (elementExists) {
      ...
      // Fait les filtres dans la toolbar (people, bouffe, etc")
...
function fn_emojiElementsToPanel(type, panel, quill) {
  // console.log('ici :', type,panel,quill);
  ...
  result.map((emoji) => {
    ...
    const t = document.createTextNode(emoji.item.shortname);
    span.appendChild(t);
